...
public:
    // public methods
    RoboClaw(const char *port, uint32_t tout);
//#ifdef __AVR__
...
//	virtual int available();
    ...
    //int peek();
    uint8_t read_byte();
    uint8_t read_byte_timeout(uint32_t timeout);
    //bool listen();
...
private:
    ...
    uint16_t crc_get();
    bool write_byte_n(uint8_t byte,...);
    bool read_byte_n(uint8_t byte,uint8_t address,uint8_t cmd,...);
