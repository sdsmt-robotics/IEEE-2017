...
// Constructor
//
RoboClaw::RoboClaw(const char *port, uint32_t tout)
{
    timeout = tout;
    strcpy(robo_port, port);
}
...
uint8_t RoboClaw::begin(long speed)
{
    robo_fd = serial_port_init(robo_port, speed);
    if(robo_fd <= 0)
    {
        std::cout << "Unable to open RoboClaw on the port "
                  << robo_port << "with baud " << speed << std::endl;
        return 1;
    }
    return 0;
}
...
uint8_t RoboClaw::write_byte(uint8_t byte)
{
    uint8_t temp = byte;
    uint8_t written = 0;
    written = write(robo_fd, &temp, sizeof(temp));
// 	if(hserial)
// 		return hserial->write_byte(byte);
// #ifdef __AVR__
// 	else
// 		return sserial->write_byte(byte);
//#endif
    return written;
}

uint8_t RoboClaw::read_byte()
{
    uint8_t temp = 0;
    ssize_t n = read(robo_fd, &temp, sizeof(temp));
    // make g++ stop complaining
    n += 1;
// 	if(hserial)
// 		return hserial->read_byte_timeout();
// #ifdef __AVR__
// 	else
// 		return sserial->read_byte_timeout();
// #endif
    return temp;
}

// uint32_t RoboClaw::available()
// {
// 	return robo_fd;
// // 	if(hserial)
...
uint8_t RoboClaw::read_byte_timeout(uint32_t timeout) // in milliseconds;
{
    uint32_t timeout_u = timeout*1000;
    clock_t curr = clock();
    uint8_t num = 0;
    uint8_t temp = 0;
    while((clock() - curr) < timeout_u)
    {
        num = read(robo_fd, &temp, sizeof(temp));
        if(num > 0)
            return temp;
    }
...
void RoboClaw::clear()
{
    ...
    // 	if(hserial){
    // 		while(hserial->available())
    // 			hserial->read_byte_timeout();
    // 	}
    // #ifdef __AVR__
    // 	else{
    // 		while(sserial->available())
    // 			sserial->read_byte_timeout();
    // 	}
    // #endif
}
...
bool RoboClaw::write_byte_n(uint8_t cnt, ... )
{
    ...
    do
    {
        ...
        write_byte(crc>>8);
        write_byte(crc);
        if(read_byte_timeout(timeout)==0xFF)
            return true;
    }
    while(trys--);
    return false;
...
bool RoboClaw::read_byte_n(uint8_t cnt,uint8_t address,uint8_t cmd,...)
{
    uint32_t value=0;
    uint8_t trys=MAXRETRY;
    int16_t data;
    do
    {
        ...
        for(uint8_t index=0; index<cnt; index++)
        {
            ...
            if(data!=-1)
            {
                data = read_byte_timeout(timeout);
                crc_update(data);
...
{
    return write_byte_n(2,address,RESETENC);
}

bool RoboClaw::ReadVersion(uint8_t address,char *version)
{
    // I changed this from uint8_t to int8_t because you're comparing against -1 down below...
    int8_t data;
    uint8_t trys=MAXRETRY;
    do
    {
...
{
    return write_byte_n(6,address,SETM1ENCCOUNT,SetDWORDval(val));
}

bool RoboClaw::SetEncM2(uint8_t address, int32_t val)
{
    return write_byte_n(6,address,SETM2ENCCOUNT,SetDWORDval(val));
}
...
{
    return write_byte_n(3,address,SETMINLB,voltage);
}

bool RoboClaw::SetMaxVoltageLogicBattery(uint8_t address, uint8_t voltage)
{
    return write_byte_n(3,address,SETMAXLB,voltage);
}
...
bool RoboClaw::SetM1VelocityPID(uint8_t address, float kp_fp, float ki_fp, float kd_fp, uint32_t qpps)
{
    ...
    return write_byte_n(18,address,SETM1PID,SetDWORDval(kd),SetDWORDval(kp),SetDWORDval(ki),SetDWORDval(qpps));
}
...
bool RoboClaw::SetM2VelocityPID(uint8_t address, float kp_fp, float ki_fp, float kd_fp, uint32_t qpps)
{
    ...
    uint32_t ki = ki_fp*65536;
    uint32_t kd = kd_fp*65536;
    return write_byte_n(18,address,SETM2PID,SetDWORDval(kd),SetDWORDval(kp),SetDWORDval(ki),SetDWORDval(qpps));
}
...
{
    return write_byte_n(4,address,M1DUTY,SetWORDval(duty));
}

bool RoboClaw::DutyM2(uint8_t address, uint16_t duty)
{
    return write_byte_n(4,address,M2DUTY,SetWORDval(duty));
}

bool RoboClaw::DutyM1M2(uint8_t address, uint16_t duty1, uint16_t duty2)
{
    return write_byte_n(6,address,MIXEDDUTY,SetWORDval(duty1),SetWORDval(duty2));
}

bool RoboClaw::SpeedM1(uint8_t address, uint32_t speed)
{
    return write_byte_n(6,address,M1SPEED,SetDWORDval(speed));
}

bool RoboClaw::SpeedM2(uint8_t address, uint32_t speed)
{
    return write_byte_n(6,address,M2SPEED,SetDWORDval(speed));
}

bool RoboClaw::SpeedM1M2(uint8_t address, uint32_t speed1, uint32_t speed2)
{
    return write_byte_n(10,address,MIXEDSPEED,SetDWORDval(speed1),SetDWORDval(speed2));
}

bool RoboClaw::SpeedAccelM1(uint8_t address, uint32_t accel, uint32_t speed)
{
    return write_byte_n(10,address,M1SPEEDACCEL,SetDWORDval(accel),SetDWORDval(speed));
}

bool RoboClaw::SpeedAccelM2(uint8_t address, uint32_t accel, uint32_t speed)
{
    return write_byte_n(10,address,M2SPEEDACCEL,SetDWORDval(accel),SetDWORDval(speed));
}
bool RoboClaw::SpeedAccelM1M2(uint8_t address, uint32_t accel, uint32_t speed1, uint32_t speed2)
{
    return write_byte_n(14,address,MIXEDSPEEDACCEL,SetDWORDval(accel),SetDWORDval(speed1),SetDWORDval(speed2));
}

bool RoboClaw::SpeedDistanceM1(uint8_t address, uint32_t speed, uint32_t distance, uint8_t flag)
{
    return write_byte_n(11,address,M1SPEEDDIST,SetDWORDval(speed),SetDWORDval(distance),flag);
}

bool RoboClaw::SpeedDistanceM2(uint8_t address, uint32_t speed, uint32_t distance, uint8_t flag)
{
    return write_byte_n(11,address,M2SPEEDDIST,SetDWORDval(speed),SetDWORDval(distance),flag);
}
...
{
    return write_byte_n(19,address,MIXEDSPEEDDIST,SetDWORDval(speed1),SetDWORDval(distance1),SetDWORDval(speed2),SetDWORDval(distance2),flag);
}

bool RoboClaw::SpeedAccelDistanceM1(uint8_t address, uint32_t accel, uint32_t speed, uint32_t distance, uint8_t flag)
{
    return write_byte_n(15,address,M1SPEEDACCELDIST,SetDWORDval(accel),SetDWORDval(speed),SetDWORDval(distance),flag);
}

bool RoboClaw::SpeedAccelDistanceM2(uint8_t address, uint32_t accel, uint32_t speed, uint32_t distance, uint8_t flag)
{
    return write_byte_n(15,address,M2SPEEDACCELDIST,SetDWORDval(accel),SetDWORDval(speed),SetDWORDval(distance),flag);
}
...
{
    return write_byte_n(23,address,MIXEDSPEEDACCELDIST,SetDWORDval(accel),SetDWORDval(speed1),SetDWORDval(distance1),SetDWORDval(speed2),SetDWORDval(distance2),flag);
}
...
{
    return write_byte_n(18,address,MIXEDSPEED2ACCEL,SetDWORDval(accel1),SetDWORDval(speed1),SetDWORDval(accel2),SetDWORDval(speed2));
}
...
{
    return write_byte_n(27,address,MIXEDSPEED2ACCELDIST,SetDWORDval(accel1),SetDWORDval(speed1),SetDWORDval(distance1),SetDWORDval(accel2),SetDWORDval(speed2),SetDWORDval(distance2),flag);
}

bool RoboClaw::DutyAccelM1(uint8_t address, uint16_t duty, uint32_t accel)
{
    return write_byte_n(8,address,M1DUTYACCEL,SetWORDval(duty),SetDWORDval(accel));
}

bool RoboClaw::DutyAccelM2(uint8_t address, uint16_t duty, uint32_t accel)
{
    return write_byte_n(8,address,M2DUTYACCEL,SetWORDval(duty),SetDWORDval(accel));
}

bool RoboClaw::DutyAccelM1M2(uint8_t address, uint16_t duty1, uint32_t accel1, uint16_t duty2, uint32_t accel2)
{
    return write_byte_n(14,address,MIXEDDUTYACCEL,SetWORDval(duty1),SetDWORDval(accel1),SetWORDval(duty2),SetDWORDval(accel2));
}

bool RoboClaw::ReadM1VelocityPID(uint8_t address,float &Kp_fp,float &Ki_fp,float &Kd_fp,uint32_t &qpps)
{
    uint32_t Kp,Ki,Kd;
    bool valid = read_byte_n(4,address,READM1PID,&Kp,&Ki,&Kd,&qpps);
...
{
    uint32_t Kp,Ki,Kd;
    bool valid = read_byte_n(4,address,READM2PID,&Kp,&Ki,&Kd,&qpps);
    Kp_fp = ((float)Kp)/65536;
    Ki_fp = ((float)Ki)/65536;
    Kd_fp = ((float)Kd)/65536;
    return valid;
...
{
    return write_byte_n(6,address,SETMAINVOLTAGES,SetWORDval(min),SetWORDval(max));
}

bool RoboClaw::SetLogicVoltages(uint8_t address,uint16_t min,uint16_t max)
{
    return write_byte_n(6,address,SETLOGICVOLTAGES,SetWORDval(min),SetWORDval(max));
}
...
bool RoboClaw::SetM1PositionPID(uint8_t address,float kp_fp,float ki_fp,float kd_fp,uint32_t kiMax,uint32_t deadzone,uint32_t min,uint32_t max)
{
    ...
    return write_byte_n(30,address,SETM1POSPID,SetDWORDval(kd),SetDWORDval(kp),SetDWORDval(ki),SetDWORDval(kiMax),SetDWORDval(deadzone),SetDWORDval(min),SetDWORDval(max));
}
...
bool RoboClaw::SetM2PositionPID(uint8_t address,float kp_fp,float ki_fp,float kd_fp,uint32_t kiMax,uint32_t deadzone,uint32_t min,uint32_t max)
{
    ...
    uint32_t ki=ki_fp*1024;
    uint32_t kd=kd_fp*1024;
    return write_byte_n(30,address,SETM2POSPID,SetDWORDval(kd),SetDWORDval(kp),SetDWORDval(ki),SetDWORDval(kiMax),SetDWORDval(deadzone),SetDWORDval(min),SetDWORDval(max));
}
...
{
    uint32_t Kp,Ki,Kd;
    bool valid = read_byte_n(7,address,READM1POSPID,&Kp,&Ki,&Kd,&KiMax,&DeadZone,&Min,&Max);
    Kp_fp = ((float)Kp)/1024;
    Ki_fp = ((float)Ki)/1024;
    Kd_fp = ((float)Kd)/1024;
    return valid;
...
{
    uint32_t Kp,Ki,Kd;
    bool valid = read_byte_n(7,address,READM2POSPID,&Kp,&Ki,&Kd,&KiMax,&DeadZone,&Min,&Max);
    Kp_fp = ((float)Kp)/1024;
    Ki_fp = ((float)Ki)/1024;
    Kd_fp = ((float)Kd)/1024;
    return valid;
...
{
    return write_byte_n(19,address,M1SPEEDACCELDECCELPOS,SetDWORDval(accel),SetDWORDval(speed),SetDWORDval(deccel),SetDWORDval(position),flag);
}
...
{
    return write_byte_n(19,address,M2SPEEDACCELDECCELPOS,SetDWORDval(accel),SetDWORDval(speed),SetDWORDval(deccel),SetDWORDval(position),flag);
}
...
{
    return write_byte_n(35,address,MIXEDSPEEDACCELDECCELPOS,SetDWORDval(accel1),SetDWORDval(speed1),SetDWORDval(deccel1),SetDWORDval(position1),SetDWORDval(accel2),SetDWORDval(speed2),SetDWORDval(deccel2),SetDWORDval(position2),flag);
}

bool RoboClaw::SetM1DefaultAccel(uint8_t address, uint32_t accel)
{
    return write_byte_n(6,address,SETM1DEFAULTACCEL,SetDWORDval(accel));
}

bool RoboClaw::SetM2DefaultAccel(uint8_t address, uint32_t accel)
{
    return write_byte_n(6,address,SETM2DEFAULTACCEL,SetDWORDval(accel));
}

bool RoboClaw::SetPinFunctions(uint8_t address, uint8_t S3mode, uint8_t S4mode, uint8_t S5mode)
{
    return write_byte_n(5,address,SETPINFUNCTIONS,S3mode,S4mode,S5mode);
}
...
{
    return write_byte_n(4,address,SETDEADBAND,Min,Max);
}
...
{
    bool valid = read_byte_n(2,address,GETENCODERS,&enc1,&enc2);
    return valid;
}

bool RoboClaw::ReadISpeeds(uint8_t address,uint32_t &ispeed1,uint32_t &ispeed2)
{
    bool valid = read_byte_n(2,address,GETISPEEDS,&ispeed1,&ispeed2);
    return valid;
}

bool RoboClaw::RestoreDefaults(uint8_t address)
{
    return write_byte_n(2,address,RESTOREDEFAULTS);
}
...
{
    return write_byte_n(3,address,SETM1ENCODERMODE,mode);
}

bool RoboClaw::SetM2EncoderMode(uint8_t address,uint8_t mode)
{
    return write_byte_n(3,address,SETM2ENCODERMODE,mode);
}

bool RoboClaw::WriteNVM(uint8_t address)
{
    return write_byte_n(6,address,WRITENVM, SetDWORDval(0xE22EAB7A) );
}

bool RoboClaw::ReadNVM(uint8_t address)
{
    return write_byte_n(2,address,READNVM);
}

bool RoboClaw::SetConfig(uint8_t address, uint16_t config)
{
    return write_byte_n(4,address,SETCONFIG,SetWORDval(config));
}
...
{
    return write_byte_n(10,address,SETM1MAXCURRENT,SetDWORDval(max),SetDWORDval(0));
}

bool RoboClaw::SetM2MaxCurrent(uint8_t address,uint32_t max)
{
    return write_byte_n(10,address,SETM2MAXCURRENT,SetDWORDval(max),SetDWORDval(0));
}

bool RoboClaw::ReadM1MaxCurrent(uint8_t address,uint32_t &max)
{
    uint32_t tmax,dummy;
    bool valid = read_byte_n(2,address,GETM1MAXCURRENT,&tmax,&dummy);
    if(valid)
        max = tmax;
    return valid;
}
...
{
    uint32_t tmax,dummy;
    bool valid = read_byte_n(2,address,GETM2MAXCURRENT,&tmax,&dummy);
    if(valid)
        max = tmax;
    return valid;
}
...
{
    return write_byte_n(3,address,SETPWMMODE,mode);
}
